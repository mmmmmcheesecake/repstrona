(function () {
    const LANGS = ['en', 'pl'];
    const KEY = 'lang';
    const DEFAULT = 'en';

    const DICT = {
        en: {
            'nav.discord': 'Discord',
            'nav.usfans': 'Claim coupons',
            'gender.men': 'Men',
            'gender.women': 'Women',
            'title.imageSearch': 'Image search — RePluG24',
            'vs.heading': 'Search by image',
            'vs.subtitle': 'Upload a photo and find similar products on Weidian or Taobao.',
            'vs.cropHint': 'Drag across the photo to search just that part of it.',
            'vs.drop': 'Drop an image here or click to upload',
            'vs.search': 'Search',
            'vs.reset': 'Reset',
            'vs.dropPaste': 'Tap here to paste an image',
            'vs.dropFile': 'or choose a file',
            'vs.linkLabel': 'Have a link instead?',
            'vs.linkPlaceholder': 'Paste a product link from any agent',
            'vs.linkOpen': 'Open',
            'vs.linkNote': 'Weidian, Taobao, 1688 or any agent link — it opens here as a normal product.',
            'vs.linkChecking': 'Checking the link…',
            'vs.linkUnknown': 'That link is not a product we can open. Paste the full link to the item.',
            'vs.linkFailed': 'Could not check the link. Try again.',
            'vs.pasteEmpty': 'No image or product link in the clipboard — copy one first.',
            'vs.pasteUnsupported': 'This browser will not share the clipboard — upload a file or paste a link below.',
            'vs.pasteDenied': 'The browser would not hand over the clipboard. Allow it, or use the upload button.',
            'vs.searching': 'Searching…',
            'vs.empty': 'No matches found in this channel.',
            'vs.error': 'Search failed. Try another image.',
            'vs.needImage': 'Drop an image first.',
            'vs.results': '{n} matches',
            'vs.results.one': '{n} match',
            'vs.byImage': 'Search by image',
            'vs.ch.weidian': 'Weidian',
            'vs.ch.taobao': 'Taobao',
            'nav.lang.label': 'Language',
            'nav.cur.label': 'Currency',
            'hero.headline.main': '20000+ products',
            'hero.headline.sub': 'with QC photos',
            'hero.sub': 'Verified replicas from trusted sellers. Always the best batch. Updated catalog with agent links and much more!',
            'hero.cta.badge': '+$800',
            'hero.cta.text': 'in USFans coupons',
            'filter.search': 'Search products, batches...',
            'sort.default': 'Default',
            'sort.priceAsc': 'Price: low to high',
            'sort.priceDesc': 'Price: high to low',
            'sort.nameAsc': 'Name: A–Z',
            'state.errorProducts': 'Failed to load products. Please refresh the page.',
            'state.errorProduct': 'Failed to load product.',
            'state.empty': 'No results. Try a different search or category.',
            'empty.hint': 'Not in the catalogue? Search with a photo, or tell us on Discord what you are looking for.',
            'empty.ask': 'Ask on Discord',
            'state.errorBack': 'Back to catalog',
            'footer.text': '© 2026 RePluG24 · replug24.com — For informational purposes only.',
            'tab.all': 'All',
            'results.count': '{n} products',
            'results.count.one': '{n} product',
            'results.close': 'nothing matches “{q}” exactly, showing the closest',
            'pd.back': '← Back',
            'pd.outOfStock': 'Out of stock',
            'pd.colorVariant': 'Color / Variant',
            'pd.size': 'Size',
            'pd.option': 'Option {n}',
            'pd.buy': 'Buy from agent →',
            'pd.buyResolving': 'Finding agent link…',
            'pd.buyUnavailable': 'No agent link for this album',
            'pd.budget': 'Budget version',
            'pd.qc': 'Quality check',
            'pd.shop': 'Shop: {name}',
            'title.catalog': 'Catalog — RePluG24',
            'title.home': 'RePluG24 — Replica Finds & QC Photos',
            'title.product': 'Product — RePluG24',
            'title.productNamed': '{name} — RePluG24',
            'title.qc': 'Quality check — RePluG24',
            'qc.heading': 'Quality check',
            'qc.subtitle': 'Paste any product link to find its QC photos.',
            'qc.placeholder': 'Paste product link (Weidian, Taobao, 1688 or agent link)…',
            'qc.check': 'Check',
            'qc.checking': 'Searching for QC photos…',
            'qc.empty': 'No QC photos found for this product yet.',
            'qc.error': 'Failed to load QC photos. Try another link.',
            'qc.unavailable': 'QC photos are temporarily unavailable — the site hosting them is not responding.',
            'qc.invalid': 'Could not recognize the product link.',
            'sellers.more': 'Show more from this shop',
            'sellers.loading': 'Loading…',
            'sellers.error': 'Could not load this shop right now. Try again in a moment.',
            'qc.badge': 'QC',
            'qc.badgeTitle': 'QC photos available',
            'qc.results': '{n} QC photos',
            'qc.results.one': '{n} QC photo',
            'qc.source': 'Source: {name}',
            'qc.setName': 'QC Photos set #{n}',
            'qc.days': 'days',
            'qc.openUsfans': 'Check on USFans',
            'qc.openAgent': 'Open at agent',
            'cart.title': 'Cart — RePluG24',
            'cart.nav': 'Cart',
            'cart.heading': 'Cart',
            'cart.empty': 'Your cart is empty.',
            'cart.browse': 'Browse catalog',
            'cart.continue': '← Continue shopping',
            'cart.add': 'Add to cart',
            'cart.added': 'Added to cart',
            'cart.share': 'Copy share link',
            'cart.shared': 'Link copied',
            'cart.copy': 'Copy',
            'cart.copyManual': 'Copy the link manually',
            'cart.clear': 'Clear cart',
            'cart.clearConfirm': 'Clear the cart?',
            'cart.remove': 'Remove',
            'cart.viewing': 'Viewing shared cart',
            'cart.clone': 'Save to my cart',
            'cart.total': 'Subtotal',
            'cart.weight': 'Approx. weight',
            'cart.shipTo': 'Ship to',
            'cart.shipping': 'Shipping (est.)',
            'cart.estimateInfo': 'Values are approximate. The actual weight and shipping cost may differ slightly.',
            'cart.estimateInfoLabel': 'More info about the estimate',
            'cart.cheaper.title': 'Buy cheaper',
            'cart.cheaper.subtitle': 'Save up to 60% on shipping',
            'cart.cheaper.modalTitle': 'How to get a cheaper proxy order',
            'cart.cheaper.step1': 'Join our Discord server.',
            'cart.cheaper.step1Cta': 'Open Discord invite',
            'cart.cheaper.step2': 'DM mmmmmcheesecake with a link to your cart or your question.',
            'cart.cheaper.step2Cta': 'Copy share link to cart',
            'cart.cheaper.step3': 'Settle the details and enjoy a cheaper package — shipping up to 60% off!',
            'cart.cheaper.close': 'Close',
            'cart.grandTotal': 'Grand total (est.)',
            'cart.region.PL': 'Poland',
            'cart.region.EU': 'Other EU',
            'cart.region.UK': 'United Kingdom',
            'cart.region.NA': 'USA / Canada',
            'cart.region.WORLD': 'Rest of world',
            'card.add': '+ Cart',
            'usfansPopup.eyebrow': 'Current promotion',
            'cart.coupon.use': 'Use coupon −{pct}%',
            'cart.coupon.claim': 'Claim the coupon',
            'cart.coupon.note': '−{pct}% shipping applied to the estimate',
            'usfansPopup.title': 'Claim +$800 in USFans coupons & 40% off shipping',
            'usfansPopup.body': 'Register an account using our link and unlock free coupons plus a 40% shipping discount on your orders. Or use code "MGRSBE" when signing up.',
            'usfansPopup.cta': 'Claim coupons',
            'usfansPopup.dismiss': 'Maybe later',
            'usfansPopup.close': 'Close',
            'cat.Sneakers': 'Sneakers',
            'cat.Hoodies/Crewnecks': 'Hoodies/Crewnecks',
            'cat.T-shirts': 'T-shirts',
            'cat.Jackets': 'Jackets',
            'cat.Pants': 'Pants',
            'cat.Shorts': 'Shorts',
            'cat.Accesories': 'Accessories',
            'cat.Watches': 'Watches',
            'cat.High-end': 'High-end',
            'cat.Underwear': 'Underwear',
            'cat.Sport Clothing': 'Sports Clothing',
            "cat.Jersey's": 'Jerseys',
            'cat.Football': 'Soccer',
            'cat.Basketball': 'Basketball',
            'cat.Lego': 'Lego',
            'cat.MISC': 'MISC',
            'cat.Vests': 'Vests',
            'cat.Other': 'Other',
            'cat.Sellers': 'Sellers',
            'sellers.count': '{n} sellers',
            'sellers.count.one': '{n} seller',
            'sellers.items': '~{n} items',
            'sellers.items.one': '~{n} item',
            'sellers.back': 'All sellers',
            'sellers.failed': 'Could not load the sellers. Tap “Sellers” to try again.',
            'sellers.view': 'View shop →',
            'sellers.tag': 'Seller',
            'sellers.found': '{n} sellers',
            'sellers.found.one': '{n} seller',
            'cat.Shorts/Skirts': 'Shorts/Skirts',
            'cat.Bags': 'Bags',
            'cat.Swimwear': 'Swimwear',
            'cat.women.Sneakers': 'Sneakers/Footwear',
        },
        pl: {
            'nav.discord': 'Discord',
            'nav.usfans': 'Odbierz kupony',
            'gender.men': 'Męskie',
            'gender.women': 'Damskie',
            'title.imageSearch': 'Wyszukiwanie po zdjęciu — RePluG24',
            'vs.heading': 'Wyszukaj po zdjęciu',
            'vs.subtitle': 'Wgraj zdjęcie żeby znaleźć podobne produkty na Weidian lub Taobao.',
            'vs.cropHint': 'Przeciągnij po zdjęciu, żeby szukać samego przedmiotu.',
            'vs.drop': 'Upuść zdjęcie tutaj lub kliknij żeby wgrać',
            'vs.search': 'Szukaj',
            'vs.reset': 'Wyczyść',
            'vs.dropPaste': 'Naciśnij tutaj, żeby wkleić zdjęcie',
            'vs.dropFile': 'albo wybierz plik',
            'vs.linkLabel': 'Masz link zamiast zdjęcia?',
            'vs.linkPlaceholder': 'Wklej link do produktu od dowolnego agenta',
            'vs.linkOpen': 'Otwórz',
            'vs.linkNote': 'Weidian, Taobao, 1688 albo link od dowolnego agenta — otworzy się tutaj jak zwykły produkt.',
            'vs.linkChecking': 'Sprawdzam link…',
            'vs.linkUnknown': 'Nie rozpoznaję tego linku. Wklej pełny link do przedmiotu.',
            'vs.linkFailed': 'Nie udało się sprawdzić linku. Spróbuj jeszcze raz.',
            'vs.pasteEmpty': 'W schowku nie ma zdjęcia ani linku do produktu — najpierw coś skopiuj.',
            'vs.pasteUnsupported': 'Ta przeglądarka nie udostępnia schowka — wgraj plik albo wklej link niżej.',
            'vs.pasteDenied': 'Przeglądarka nie dała dostępu do schowka. Zezwól albo wgraj plik.',
            'vs.searching': 'Szukam…',
            'vs.empty': 'Brak podobnych produktów w tym kanale.',
            'vs.error': 'Wyszukiwanie nie udało się. Spróbuj innego zdjęcia.',
            'vs.needImage': 'Najpierw wgraj zdjęcie.',
            'vs.results': 'Znaleziono {n} produktów',
            'vs.results.one': 'Znaleziono {n} produkt',
            'vs.results.few': 'Znaleziono {n} produkty',
            'vs.byImage': 'Szukaj po zdjęciu',
            'vs.ch.weidian': 'Weidian',
            'vs.ch.taobao': 'Taobao',
            'nav.lang.label': 'Język',
            'nav.cur.label': 'Waluta',
            'hero.headline.main': '20000+ produktów',
            'hero.headline.sub': 'ze zdjęciami QC',
            'hero.sub': 'Sprawdzone repliki od zaufanych sellerów. Zawsze najlepszy batch. Aktualizowany katalog z linkami do agentów i wiele więcej!',
            'hero.cta.badge': '+800 USD',
            'hero.cta.text': 'w kuponach USFans',
            'filter.search': 'Szukaj produktów, batchy...',
            'sort.default': 'Domyślnie',
            'sort.priceAsc': 'Cena: od najniższej',
            'sort.priceDesc': 'Cena: od najwyższej',
            'sort.nameAsc': 'Nazwa: A–Z',
            'state.errorProducts': 'Nie udało się załadować produktów. Odśwież stronę.',
            'state.errorProduct': 'Nie udało się załadować produktu.',
            'state.empty': 'Brak wyników. Spróbuj innego wyszukiwania lub kategorii.',
            'empty.hint': 'Nie ma tego w katalogu? Poszukaj po zdjęciu albo napisz na Discordzie, czego szukasz.',
            'empty.ask': 'Zapytaj na Discordzie',
            'state.errorBack': 'Wróć do katalogu',
            'footer.text': '© 2026 RePluG24 · replug24.com — Wyłącznie w celach informacyjnych.',
            'tab.all': 'Wszystkie',
            'results.count': '{n} produktów',
            'results.count.one': '{n} produkt',
            'results.count.few': '{n} produkty',
            'results.close': 'nic nie pasuje dokładnie do „{q}”, pokazujemy najbliższe',
            'pd.back': '← Wróć',
            'pd.outOfStock': 'Brak w magazynie',
            'pd.colorVariant': 'Kolor / Wariant',
            'pd.size': 'Rozmiar',
            'pd.option': 'Opcja {n}',
            'pd.buy': 'Kup u agenta →',
            'pd.buyResolving': 'Szukam linku do agenta…',
            'pd.buyUnavailable': 'Brak linku do agenta dla tego albumu',
            'pd.budget': 'Wersja budżetowa',
            'pd.qc': 'Sprawdź jakość',
            'pd.shop': 'Sklep: {name}',
            'title.catalog': 'Katalog — RePluG24',
            'title.home': 'RePluG24 — Repliki i zdjęcia QC',
            'title.product': 'Produkt — RePluG24',
            'title.productNamed': '{name} — RePluG24',
            'title.qc': 'Sprawdź jakość — RePluG24',
            'qc.heading': 'Sprawdź jakość',
            'qc.subtitle': 'Wklej link do produktu, żeby znaleźć zdjęcia QC.',
            'qc.placeholder': 'Wklej link produktu (Weidian, Taobao, 1688 lub link agenta)…',
            'qc.check': 'Sprawdź',
            'qc.checking': 'Szukam zdjęć QC…',
            'qc.empty': 'Brak zdjęć QC dla tego produktu.',
            'qc.error': 'Nie udało się pobrać zdjęć QC. Spróbuj innego linku.',
            'qc.unavailable': 'Zdjęcia QC są chwilowo niedostępne — serwer, który je hostuje, nie odpowiada.',
            'qc.invalid': 'Nie rozpoznano linku produktu.',
            'sellers.more': 'Pokaż więcej z tego sklepu',
            'sellers.loading': 'Wczytuję…',
            'sellers.error': 'Nie udało się teraz pobrać tego sklepu. Spróbuj za chwilę.',
            'qc.badge': 'QC',
            'qc.badgeTitle': 'Są zdjęcia QC',
            'qc.results': 'Znaleziono {n} zdjęć QC',
            'qc.results.one': 'Znaleziono {n} zdjęcie QC',
            'qc.results.few': 'Znaleziono {n} zdjęcia QC',
            'qc.source': 'Źródło: {name}',
            'qc.setName': 'Zestaw zdjęć QC #{n}',
            'qc.days': 'dni',
            'qc.openUsfans': 'Sprawdź na USFans',
            'qc.openAgent': 'Otwórz u agenta',
            'cart.title': 'Koszyk — RePluG24',
            'cart.nav': 'Koszyk',
            'cart.heading': 'Koszyk',
            'cart.empty': 'Twój koszyk jest pusty.',
            'cart.browse': 'Przeglądaj katalog',
            'cart.continue': '← Wróć do zakupów',
            'cart.add': 'Dodaj do koszyka',
            'cart.added': 'Dodano do koszyka',
            'cart.share': 'Skopiuj link do koszyka',
            'cart.shared': 'Link skopiowany',
            'cart.copy': 'Kopiuj',
            'cart.copyManual': 'Skopiuj link ręcznie',
            'cart.clear': 'Wyczyść koszyk',
            'cart.clearConfirm': 'Wyczyścić koszyk?',
            'cart.remove': 'Usuń',
            'cart.viewing': 'Przeglądasz udostępniony koszyk',
            'cart.clone': 'Zapisz do mojego koszyka',
            'cart.total': 'Suma produktów',
            'cart.weight': 'Przybliżona waga',
            'cart.shipTo': 'Wysyłka do',
            'cart.shipping': 'Dostawa (szacunek)',
            'cart.estimateInfo': 'Wartości są przybliżone. Realna waga oraz koszt dostawy mogą się trochę różnić.',
            'cart.estimateInfoLabel': 'Więcej informacji o szacunku',
            'cart.cheaper.title': 'Kup taniej',
            'cart.cheaper.subtitle': 'Oszczędź nawet 60% na dostawie',
            'cart.cheaper.modalTitle': 'Jak zrobić tańsze proxy',
            'cart.cheaper.step1': 'Dołącz na nasz serwer Discord.',
            'cart.cheaper.step1Cta': 'Otwórz zaproszenie na Discord',
            'cart.cheaper.step2': 'Napisz DM do mmmmmcheesecake z linkiem do swojego koszyka lub pytaniem.',
            'cart.cheaper.step2Cta': 'Skopiuj link do koszyka',
            'cart.cheaper.step3': 'Ustal szczegóły i ciesz się tańszą paczką z przesyłką tańszą nawet o 60%!',
            'cart.cheaper.close': 'Zamknij',
            'cart.grandTotal': 'Łączny koszt (szacunek)',
            'cart.region.PL': 'Polska',
            'cart.region.EU': 'Inne kraje UE',
            'cart.region.UK': 'Wielka Brytania',
            'cart.region.NA': 'USA / Kanada',
            'cart.region.WORLD': 'Reszta świata',
            'card.add': '+ Koszyk',
            'usfansPopup.eyebrow': 'Obecna promocja',
            'cart.coupon.use': 'Użyj kuponu −{pct}%',
            'cart.coupon.claim': 'Odbierz kupon',
            'cart.coupon.note': 'Naliczono −{pct}% na wysyłkę w wycenie',
            'usfansPopup.title': 'Odbierz +800 USD w kuponach USFans i 40% zniżki na wysyłkę',
            'usfansPopup.body': 'Załóż konto z naszego linku i odbierz darmowe kupony oraz 40% zniżki na wysyłkę. Lub użyj kodu „MGRSBE" przy rejestracji.',
            'usfansPopup.cta': 'Odbierz kupony',
            'usfansPopup.dismiss': 'Może później',
            'usfansPopup.close': 'Zamknij',
            'cat.Sneakers': 'Buty',
            'cat.Hoodies/Crewnecks': 'Bluzy',
            'cat.T-shirts': 'Koszulki',
            'cat.Jackets': 'Kurtki',
            'cat.Pants': 'Spodnie',
            'cat.Shorts': 'Spodenki',
            'cat.Accesories': 'Akcesoria',
            'cat.Watches': 'Zegarki',
            'cat.High-end': 'High-end',
            'cat.Underwear': 'Bielizna',
            'cat.Sport Clothing': 'Odzież sportowa',
            "cat.Jersey's": 'Stroje sportowe',
            'cat.Football': 'Piłka nożna',
            'cat.Basketball': 'Koszykówka',
            'cat.Lego': 'Lego',
            'cat.MISC': 'Różne',
            'cat.Vests': 'Kamizelki',
            'cat.Other': 'Inne',
            'cat.Sellers': 'Sprzedawcy',
            'sellers.count': '{n} sklepów',
            'sellers.count.one': '{n} sklep',
            'sellers.count.few': '{n} sklepy',
            'sellers.items': '~{n} produktów',
            'sellers.items.one': '~{n} produkt',
            'sellers.items.few': '~{n} produkty',
            'sellers.back': 'Wszyscy sprzedawcy',
            'sellers.failed': 'Nie udało się pobrać sprzedawców. Kliknij „Sprzedawcy”, aby spróbować ponownie.',
            'sellers.view': 'Wejdź do sklepu →',
            'sellers.tag': 'Sprzedawca',
            'sellers.found': '{n} sprzedawców',
            'sellers.found.one': '{n} sprzedawca',
            'sellers.found.few': '{n} sprzedawcy',
            'cat.Shorts/Skirts': 'Spodenki/Spódnice',
            'cat.Bags': 'Torebki',
            'cat.Swimwear': 'Stroje kąpielowe',
            'cat.women.Sneakers': 'Buty/Obuwie',
        },
    };

    const listeners = [];

    // A visitor who has never picked a language gets the first one their browser asks for
    // that the site has. Most visitors are Polish, and every one of them used to land on
    // English and have to find the switch. Nothing is stored, so a choice made with the
    // switch still wins.
    function browserLang() {
        const tags = navigator.languages && navigator.languages.length
            ? navigator.languages
            : [navigator.language || ''];
        for (const tag of tags) {
            const base = String(tag).toLowerCase().split('-')[0];
            if (LANGS.includes(base)) return base;
        }
        return DEFAULT;
    }

    function getLang() {
        let v = null;
        try { v = localStorage.getItem(KEY); } catch {}
        return LANGS.includes(v) ? v : browserLang();
    }

    function setLang(l) {
        if (!LANGS.includes(l)) return;
        localStorage.setItem(KEY, l);
        applyAll();
        notify(l);
    }

    function onChange(fn) { listeners.push(fn); }

    function notify(l) {
        listeners.forEach(fn => { try { fn(l); } catch (e) { console.warn(e); } });
    }

    // Polish counts three ways — 1 produkt, 2 produkty, 5 produktów — and English two.
    // A key used with a count {n} may carry ".one" and ".few" forms beside it; the plain
    // key is the form for every other number (5 produktów, 0 products). Keys without
    // those forms, like "Option {n}", are untouched.
    function pluralSuffix(lang, n) {
        if (n === 1) return 'one';
        if (lang !== 'pl') return '';
        const units = n % 10, tens = n % 100;
        return units >= 2 && units <= 4 && (tens < 12 || tens > 14) ? 'few' : '';
    }

    function t(key, vars) {
        const lang = getLang();
        let s = null;
        const count = vars ? Number(vars.n) : NaN;
        if (Number.isInteger(count)) {
            const suffix = pluralSuffix(lang, Math.abs(count));
            if (suffix && DICT[lang]) s = DICT[lang][`${key}.${suffix}`] || null;
        }
        s = s || (DICT[lang] && DICT[lang][key]) || (DICT[DEFAULT] && DICT[DEFAULT][key]) || key;
        if (vars) {
            Object.keys(vars).forEach(k => {
                s = s.replace(new RegExp('\\{' + k + '\\}', 'g'), vars[k]);
            });
        }
        return s;
    }

    function applyAll() {
        const lang = getLang();
        document.documentElement.setAttribute('lang', lang);

        document.querySelectorAll('[data-i18n]').forEach(el => {
            el.textContent = t(el.getAttribute('data-i18n'));
        });
        document.querySelectorAll('[data-i18n-html]').forEach(el => {
            el.innerHTML = t(el.getAttribute('data-i18n-html'));
        });
        document.querySelectorAll('[data-i18n-attr]').forEach(el => {
            const spec = el.getAttribute('data-i18n-attr');
            spec.split(',').forEach(pair => {
                const [attr, key] = pair.split(':').map(s => s.trim());
                if (attr && key) el.setAttribute(attr, t(key));
            });
        });
        const titleEl = document.querySelector('title[data-i18n-title]');
        if (titleEl) document.title = t(titleEl.getAttribute('data-i18n-title'));

        const sel = document.getElementById('langSelect');
        if (sel) sel.value = lang;
    }

    function initPicker() {
        const nav = document.querySelector('.nav-right');
        if (!nav || document.getElementById('langSelect')) return;
        const sel = document.createElement('select');
        sel.id = 'langSelect';
        sel.className = 'lang-select';
        sel.setAttribute('aria-label', t('nav.lang.label'));
        [['en', 'EN'], ['pl', 'PL']].forEach(([v, label]) => {
            const o = document.createElement('option');
            o.value = v;
            o.textContent = label;
            sel.appendChild(o);
        });
        sel.value = getLang();
        sel.addEventListener('change', () => setLang(sel.value));
        nav.insertBefore(sel, nav.firstChild);
    }

    window.RePluGI18n = { t, current: getLang, set: setLang, onChange, apply: applyAll };

    function boot() {
        initPicker();
        applyAll();
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', boot);
    } else {
        boot();
    }
})();
